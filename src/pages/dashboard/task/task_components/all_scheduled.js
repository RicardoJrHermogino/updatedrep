...
import {
  Grid, Typography, Box, IconButton, Button, Dialog, DialogTitle, 
  DialogContent, DialogActions, TextField, Card, CardContent, InputBase
} from '@mui/material';
...
const AllScheduled = () => {
  ...
  const fetchTasks = async (userId) => {
    try {
      const response = await fetch(`http://localhost:3001/tasks?userId=${userId}`); // Adjust the endpoint as necessary
      if (!response.ok) {
